Feature: End-to-End Testing of Login and Redirect
  ...
  Scenario: Login and verify redirect of existent user
    Given driver "http://localhost:8082/realms/myrealm/protocol/openid-connect/auth?response_type=code&client_id=myrealm-client&state=fhihfi2312&scope=profile&redirect_uri=http://localhost:8080/opi_lab3-1.0-SNAPSHOT/index.jsp"
    And input('input[name=username]', 'maria123')
    And input('input[name=password]', 'itmo12')
 #   And submit()
    And click('#kc-form-buttons')
#    And waitForUrl('http://localhost:8080/opi_lab3-1.0-SNAPSHOT/index.jsp')
   * java.lang.Thread.sleep(5000)


  Scenario: Filling out the form and sending it
    Given driver "http://localhost:8080/opi_lab3-1.0-SNAPSHOT/index.jsp"
    And waitFor("#x_value")
    And input("#x_value", "2")
